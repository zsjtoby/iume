...
// iUMeCloud.cpp : 定义应用程序的类行为。
//

#include "stdafx.h"
#include "iUMeCloud.h"
//#include "iUMeCloudDlg.h"
#include "iCloud.h"
...
BEGIN_MESSAGE_MAP(CiUMeCloudApp, CWinApp)
...
// 唯一的一个 CiUMeCloudApp 对象

CiUMeCloudApp theApp;


// CiUMeCloudApp 初始化

BOOL CiUMeCloudApp::InitInstance()
{
	...
	CPaintManagerUI::SetResourceZip(_T("cloud.zip"));

	HRESULT Hr = ::CoInitialize(NULL);
	if( FAILED(Hr) ) return 0;

	CCloud* pFrame = new CCloud();
	if( pFrame == NULL ) return 0;
	pFrame->Create(NULL, _T("爱天气"), UI_WNDSTYLE_FRAME, WS_EX_WINDOWEDGE);	
...
void CiUMeCloudApp::SetActiveWindow(HWND hWndActive)
{
...
BOOL CiUMeCloudApp::CheckInstance()
{
	BOOL bRet = FALSE;
	HRESULT hRet = E_FAIL;
